...
namespace IR
{
    ...
    void RX_data_fragments_pool::Reset_pool_data(Trans_info info)
    {
        ...
        if (msg.msg_ID)
        {
            ...
            data_pool[msg.msg_ID - 1]=msg.content;
        }
        else
        {
            Msg_header_t msg_hd;
    ...
    bool RX_data_fragments_pool::Add_element(Trans_info info)
    {
        ...
        if ((!msg_count && !msg_ID_max)                                                                              // 0. no data
            ...
            || (prev_filled && (data_pool[msg.msg_ID - 1] != msg.content)) // 4. data inconsistent
            || header_invalid)                                                                                       // 5. header invalid
        {
            DEBUG_C(
                ...
                else if (prev_filled && (data_pool[msg.msg_ID - 1] != msg.content)) {
                    Serial.println("Msg content inconsistent reset");
                    Serial.print("Old content: ");
                    Serial.println(data_pool[msg.msg_ID - 1]);
    ...
    RMT_TX *RMT_RX_TX::TX_prep_1 = nullptr;
    RMT_TX *RMT_RX_TX::TX_prep_2 = nullptr;
